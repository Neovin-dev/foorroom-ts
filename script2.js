...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // visibilty of filtermenu acc ot width of the screen.
  if (window.innerWidth < 1025) {
    ...
    if (sortControls && sortControls.classList.contains("deactive-style"))
  ...
  function rerenderTable() {
    ...
    if (registrations.length === 0) {
      ...
      if (dataSection && !dataSection.classList.contains("deative-style"))
  ...
  registrationForm.addEventListener("submit", function (event) {
    ...
    // subject validation
    let subjectCheckboxes = Array.from(
      registrationForm.querySelectorAll(
        'input[type="checkbox"][name="maths"], input[type="checkbox"][name="english"], input[type="checkbox"][name="french"], input[type="checkbox"][name="history"]',
      ),
    );
    ...
    let currentId =
      registrations.length > 0
        ? Math.max(...registrations.map((u) => u.id)) + 1
        : 0;
    ...
    let subMarked = registrationForm.querySelectorAll(
      '.subject-selection-group input[type="checkbox"]:checked',
    );
    ...
    tableBody.addEventListener("click", function (event) {
      ...
      if (editButton) {
        ...
        genderRadios.forEach((radio) => {
          radio.checked =
            radio.parentElement.innerText.trim() === userEdit.gender;
        });

        let subjectCheckboxes = registrationForm.querySelectorAll(
          '.subject-selection-group input[type="checkbox"]',
        );
      ...
      if (deleteButton) {
        ...
        if (
          dataSection &&
          !dataSection.classList.contains("deative-style") &&
          registrations.length === 0
        )
          dataSection.classList.add("deactive-style");
        ...
        deleteMode = true;
        if (window.innerWidth < 1025) {
          if (registrations.length === 0 || filteredRegistrations.length === 0)
            mobileControls.classList.add("deactive-style");
        }
  ...
  if (applyFilterButton) {
    applyFilterButton.addEventListener("click", function () {
      ...
      let selectedGenderCheckBoxes = document.querySelectorAll(
        '#filterMenu input[name="gender-filter"]:checked',
      );
      let selectedSubjectCheckboxes = document.querySelectorAll(
        '#filterMenu input[name="subject"]:checked',
      );
      let selectedCenterCheckBoxes = document.querySelectorAll(
        '#filterMenu input[name="center"]:checked',
      );

      let selectedGenders = Array.from(selectedGenderCheckBoxes).map((cb) =>
        cb.value.toLowerCase(),
      );
      let selectedSubjects = Array.from(selectedSubjectCheckboxes).map((cb) =>
        cb.value.toLowerCase(),
      );
      let selectedCenters = Array.from(selectedCenterCheckBoxes).map((cb) =>
        cb.value.toLowerCase(),
      );
      ...
      let filteredRegistrations = registrations.filter((user) => {
        ...
        const centerMatch =
          ...
          selectedCenters.includes(user.exam.trim().toLowerCase());
        const subjectMatch =
  ...
  function applyMobileFilters() {
    let selectedGenderCheckBoxes = document.querySelectorAll(
      '#filtersOverlay input[name="gender-mobile"]:checked',
    );
    let selectedSubjectCheckboxes = document.querySelectorAll(
      '#filtersOverlay input[name="subject-mobile"]:checked',
    );
    let selectedCenterCheckBoxes = document.querySelectorAll(
      '#filtersOverlay input[name="center-mobile"]:checked',
    );
  ...
  function clearMobileFilters() {
    let allCheckBoxes = document.querySelectorAll(
      '#filtersOverlay input[type="checkbox"]',
    );
    allCheckBoxes.forEach((checkbox) => (checkbox.checked = false));
    rerenderTable();
  ...
  const mobileSortOptions = document.querySelector(
    "#sortOverlay .overlay-options",
  );
  const mobileApplyFilterButton = document.getElementById(
    "mobileApplyFilterButton",
  );
  const mobileClearFilterButton = document.getElementById(
